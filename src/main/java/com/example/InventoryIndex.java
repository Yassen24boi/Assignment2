...
public class InventoryIndex
    ...
    public void buildQuantityIndex() {
        ...
        for (int position = 0;position < inventoryTable.size();position++) {
            Inventory item = inventoryTable.get(position);
    ...
    public List<Inventory> findByPrice( // since there can be multiple records with the same price we will use the price index to get the list of positions of the records in the inventory table that have the given price and then we will return the inventory records at those positions
            double price) {
                ...
                for (Integer pos : positions) {        
                        results.add(inventoryTable.get(pos));
                }
        return results;
    }
    ...
    public void printInventorySortedByPrice() {
        ...
        for (Double price : priceIndex.keySet()) {

            System.out.println("\nPrice = " + price); // print the price

            List<Integer> positions = priceIndex.get(price); // get the list of positions of the records in the inventory table that have the given price using the price index

            for (Integer pos : positions) { //  iterate through the list of positions and print the inventory record at each position

                System.out.println(inventoryTable.get(pos)); //  print the inventory record at the current position
            }
        }
    }
    ...
    public void printInventorySortedByName() {
        ...
        for (String productName : productNameIndex.keySet()) { // iterate through the product name index using the key set which will give us the product names in sorted order since we are using a tree map
        
                System.out.println("\nProduct Name = " + productName); // print the product name
        
                Integer position = productNameIndex.get(productName);
        
                System.out.println(inventoryTable.get(position));
           }
    ...
    public void printInventorySortedByCategory() {
        ...
        for (String category : categoryIndex.keySet()) {
                ...
                List<Integer> positions = categoryIndex.get(category);
        
                for (Integer pos : positions) {
                System.out.println(inventoryTable.get(pos));
                }
    ...
    public void printInventorySortedByQuantity() {
        ...
        for (Integer quantity : quantityIndex.keySet()) {
           
                System.out.println("\nQuantity = " + quantity);
           
                List<Integer> positions = quantityIndex.get(quantity);
